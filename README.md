...
2. 재난유형에 대해
High_Urgency_Group = 5
Medium_Urgency_Group = 3
Low_Urgency_Group = 1
...
### 📄 신고 내용 요약 및 핵심 정보 추출 
- stt_result : STT 변환 (wav -> text),
- urgency_classification : 긴급도 분류 (text -> text),
- gpt_response : 요약 및 정리 (text -> text) 
...
- 모든 내용은 한국어로 작성하고, 출동 대원이 즉시 파악할 수 있도록 한다. <br>
...
반면 **개선 모델**은 비긴급 클래스(긴급도 하+중)를 묶었을 때 <br>
